...
// GET: list allowed admin emails (super admin only)
export async function GET(req: NextRequest) {
  ...
  return new Response(JSON.stringify(list.map((a: any) => ({
    _id: String(a._id),
    email: a.email,
...
// POST: add allowed admin email (super admin only)
export async function POST(req: NextRequest) {
  ...
  if (existing) return new Response(JSON.stringify({ error: "Email already allowed" }), { status: 400 });
...
// DELETE: remove an allowed email (super admin only)
